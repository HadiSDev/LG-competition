...
from stable_baselines.common import TensorboardWriter, SetVerbosity, explained_variance
...
#
# PPOAgent which sets a new level after each network update
#
class PPOAgent(PPO2):

    def __init__(self, policy, env, levels: List[int], gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4,
                 vf_coef=0.5,
                 max_grad_norm=0.5, lam=0.95, nminibatches=4, noptepochs=4, cliprange=0.2, cliprange_vf=None,
                 verbose=0, tensorboard_log=None, _init_setup_model=True, policy_kwargs=None,
                 full_tensorboard_log=False, seed=None, n_cpu_tf_sess=None, ):
        super().__init__(policy, env, gamma=gamma, n_steps=n_steps, ent_coef=ent_coef, learning_rate=learning_rate,
                         vf_coef=vf_coef,
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                # Added code here #######################
                self.env.envs[0].set_level(random.choice(self.levels))
                self.env.envs[0].reset(seed=time.time())
                #######################################
